...
Acceptor::Acceptor( Reactor_1_0::Reactor *reactor ) : EventHandler( reactor )
{
...
int Acceptor::handleInput( int fd )
{
    ...
    std::unique_ptr<ServerEventHandler> handler( new ServerEventHandler( *this ) );

    // pass the proactor's pointer
    if ( handler->proactor() == 0 )
    {
        handler->proactor( Proactor_1_0::Proactor::getInstance() );
    }
